...
data_mps$position <- NA
data_mps$position[data_mps$level == "prov"] <- "Quebec MNA"
data_mps$position[data_mps$level == "fed"] <- "Canadian MP"

data_mps$gpt_35_chars <- NA
...
for (i in 1:nrow(data_mps)) {
        ...
        data_mps$gpt_35_chars[i] <- chat_prompt$choices$message.content
